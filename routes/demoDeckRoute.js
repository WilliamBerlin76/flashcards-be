...
router.get('/:id', (req, res) => {
    ...
    const decArr = []
    Demo.getListOfDecks(id).then(collections => {
        ...
        .catch(err => {
            ...
            res.status(500).json({ error: 'There was an error retrieving the demodeck by Id from the database' })
        })
})
...
router.get('/:id/:colId', (req, res) => {
    ...
    const cardArr = [];
    Demo.getDeckById(id, colId).then(snapshot => {
        snapshot.forEach(doc => {
